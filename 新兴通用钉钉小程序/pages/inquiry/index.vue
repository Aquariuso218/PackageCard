<template>
	<view class="_content" style="padding: 5rpx 20rpx 0rpx 20rpx;">
		<!-- <view class="topBox">
			<uni-row class="uni-row">
				<uni-col style="background-color: chocolate;" class="custom-col" :span="6">
					<view class="uni-col_search">出库仓库:</view>
				</uni-col>
		...
		<view class="_content" style="padding-top: 20rpx;">
			<view v-for="(item, index) in listData" :key="item.iDLsID"
				:class="['card', {'selected': isSelected(item.iDLsID) }]" @click="toggleCancel(item.iDLsID)">
				<uni-row class="uni-row">
					<uni-col :span="8">
						<view class="uni-col textStyle">发货单号:</view>
					</uni-col>
					<uni-col :span="16" class="textRight">
						<view class="uni-col textStyle">{{ item.cDLCode }}</view>
					</uni-col>
				</uni-row>
				<uni-row class="uni-row">
					<uni-col :span="8">
				...
				<uni-row class="uni-row">
					...
					<uni-col :span="16" class="textRight">
						<view class="uni-col">{{ item.cInvCode }}</view>
					</uni-col>
				</uni-row>
				<uni-row class="uni-row">
					<uni-col :span="8">
					...
					<uni-col :span="16" class="textRight">
						<view class="uni-col">{{ item.cInvName }}</view>
					</uni-col>
				</uni-row>
				<uni-row class="uni-row">
					<uni-col :span="8">
		...
		<view class="sureBtn">
			<view style="width: 80%;">
				<u-button size="large" text="销售出库" color="#43C6AC" @click="stockIn"></u-button>
			</view>
		</view>
		...
		<u-popup :customStyle="{height: '30vh'}" :show="drawer_SX" :closeable="false" @close="drawer_SX_Close"
			...
			<view style="padding: 5px;">
				...
				<u-row customStyle="margin-bottom: 5px">
					<u-col span="3">
						<view class="demo-layout textView">发货单号:</view>
					</u-col>
					<u-col span="9">
						<view class="demo-layout">
							<u--input v-model="params.cDLCode" placeholder="请输入发货单号" />
						</view>
					</u-col>
				</u-row>
				<u-row customStyle="margin-bottom: 5px">
...
<script>
	...
	export default {
		...
		methods: {
			scanCode() {
				/*****测试用*****/
				// this.getList("成品A|250314001|20250312001/20250312002/20250312003/20250312004");
				// return;
			...
			getList(code) {
				...
				try {
					...
					const invIDs = parts[2].split('/').filter(id => id); // 提取 invID 数组
					if (!invIDs.length) {
						uni.showToast({
							title: '二维码中无有效产品id！',
							icon: 'none'
			...
			// 匹配并选中逻辑
			matchAndSelect(invIDs) {
				// 清空之前的选中状态（可选，根据需求决定是否保留）
				// this.selectedIDs = [];

				// 匹配 invIDs 和 listData 的 cBatch
				invIDs.forEach(invID => {
					const matchedItem = this.listData.find(listItem => listItem.cBatch === invID);
					if (matchedItem && !this.selectedIDs.includes(matchedItem.iDLsID)) {
						this.selectedIDs.push(matchedItem.iDLsID);
					}
				});
				...
				this.listData = [
					...this.listData.filter(item => this.selectedIDs.includes(item.iDLsID)),
					...this.listData.filter(item => !this.selectedIDs.includes(item.iDLsID))
				];
			},
			//产品出库
			async stockIn() {
				...
				//数据处理
				const stockData = this.listData
					.filter(item => this.selectedIDs.includes(item.iDLsID))
					.map(item => ({
				...
				try {
					const res = await salesInvoice({
						Cmaker: this.userInfo.name,
						DispatchDetails: stockData
					});
					if (res.code === 1) {
						uni.showToast({
							title: '产品出库成功！',
			...
			toggleCancel(id) {
				...
				if (this.selectedIDs.includes(id)) {
					const item = this.listData.find(item => item.iDLsID === id);
