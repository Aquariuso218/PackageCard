			...
			<uni-table ref="tableData" border stripe>
				<view class="tableHead">
					<uni-tr>
						...
						<uni-th width="120" align="center">存货名称</uni-th>
						<uni-th width="120" align="center">规格型号</uni-th>
						<!-- <uni-th width="70" align="center">材料</uni-th> -->
						<uni-th width="60" align="center">总计划</uni-th>
						<uni-th width="65" align="center">送货在途</uni-th>
						<uni-th width="65" align="center">在制计划</uni-th>
						<uni-th width="65" align="center">前期差缺</uni-th>
						<!-- 动态日期列 -->
				...
				<view class="tableBody">
					<uni-tr v-for="(item, rowIndex) in tableData" :key="rowIndex">
						<uni-td width="30" class="fixed-column" align="center">{{ item.cXh }}</uni-td>
						<uni-td width="120" @click="showPopup('存货名称', item.cInvName)">
							{{ item.cInvName.length > 8 ? item.cInvName.slice(0, 8) + '...' : item.cInvName }}
							<!-- {{ item.cInvName }} -->
						</uni-td>
						<uni-td width="120" @click="showPopup('规格型号', item.cInvStd)">
							{{ item.cInvStd.length > 8 ? item.cInvStd.slice(0, 8) + '...' : item.cInvStd }}
							<!-- {{ item.cInvStd }} -->
						</uni-td>
						<uni-td width="60" align="right">{{ item.cTotalQty || '' }}</uni-td>
						<uni-td width="65" align="right">{{ item.cZTQty || '' }}</uni-td>
						<uni-td width="65" align="right">{{ item.cZZQty || '' }}</uni-td>
						<uni-td width="65" align="right">
							<view class="td-full"
...
<script>
import { GetOrderByPO, invList, venList, personList } from "@/nxTemp/apis/sunset.js"
export default {
	data() {
		return {
			//筛选框样式
